...
/* Do nothing, just meant to wake up the Scout. Declared weak to allow a
 * custom sketch to still use this ISR. */
EMPTY_INTERRUPT(PCINT0_vect)
void PCINT0_vect(void) __attribute__((__weak__));

Pbbe::LogicalPin::mask_t SleepHandler::pinWakeups = 0;

static inline uint32_t read_sccnt() {
...
static inline void write_scocr3(uint32_t val) {
...
void SleepHandler::doSleep(bool interruptible) {
  ...
  // Clear any the pending PCINT0 flag
  PCIFR |= (1 << PCIF0);
